...
# ⚙️ PAGE SETUP
st.set_page_config(page_title="MAYA AI - The Final Masterpiece", layout="wide")
st.title("MAYA AI 🦅: The Final Masterpiece ⚡")
st.markdown("Aapke saare rules ek sath, ek naye aur safe dhache mein: **1. Pattern Accuracy (0/1 Fail) | 2. Timeframe Linker | 3. Tier Linker | 4. Black Box | 5. Golden VIPS!**")
...
def get_tier_name(num, tiers_dict):
    if num in tiers_dict.get('H', []): return 'H'
    elif num in tiers_dict.get('M', []): return 'M'
    elif num in tiers_dict.get('L', []): return 'L'
    return 'FAIL'

# ⬛ 3. BLACK BOX (DOOMED SEQUENCE DETECTOR)
@st.cache_data
def get_doomed_timeframe_predictions(history_tuple):
    h_list = list(history_tuple)
    black_traps = set()
    if len(h_list) < 20: return list(black_traps)
    
    for tf in range(1, 91):
        if len(h_list) - tf < 15: continue
        hit_history = []
        for i in range(15, len(h_list)):
            if i - tf < 0: continue
            pat = h_list[i-tf:i]
            nxt = [h_list[k+tf] for k in range(i-tf) if h_list[k:k+tf] == pat]
            if not nxt: hit_history.append(False)
            else:
                top = Counter(nxt).most_common(1)[0][0]
                td = get_all_tiers_cached(tuple(h_list[max(0, i-45):i]))
        ...
        if hit_history[-1] == True:
            ...
            for h in hit_history[:-curr_f]:
                if not h: f_count += 1
                    ...
                    f_count = 0
            if past_fails and curr_f < min(past_fails): 
                will_fail = True
...
# 🔗 4. TIMEFRAME LINKER & PATTERN ACCURACY
def get_unified_best_timeframe(history_tuple, dates_tuple, prev_shift_decisions):
    ...
    if zero_fail_candidates:
        best = sorted(zero_fail_candidates, key=lambda x: (-x['p_acc'], -x['p_succ'], x['max_f']))[0]
        ...
        best = sorted(one_fail_candidates, key=lambda x: (-x['p_acc'], -x['p_succ'], x['max_f']))[0]
        ...
        best = sorted(other_candidates, key=lambda x: (-x['p_acc'], -x['p_succ'], x['max_f']))[0]
...
# 🎯 5. TIER LINKER (Markov Chain)
def calculate_tier_link(curr_shift, prev_shift, prev_tier_actual, filtered_df):
    ...
    for idx, row in temp_df.iterrows():
        ...
        p_tiers = get_all_tiers_cached(tuple(p_hist[-45:]))
        c_tiers = get_all_tiers_cached(tuple(c_hist[-45:]))
...
# 🎨 6. UI RENDER ENGINE (Gold, Green, Black, Grey)
def render_ank(raw_tier_nums, traps, black_boxes, super_vips):
    all_display_nums = list(set(raw_tier_nums) | set(super_vips) | set(traps) | set(black_boxes))
    all_display_nums.sort()
    
    html = "<div style='display: flex; flex-wrap: wrap; gap: 6px; margin-top: 10px;'>"
    for n in all_display_nums:
        if n in black_boxes:
        ...
        elif n in super_vips:
            bg = "#FFD700"; border = "#DAA520"; font_c = "black"; extra = "box-shadow: 0 0 10px #FFD700; transform: scale(1.05);"
        elif n in raw_tier_nums:
        ...
        if len(filtered_df) > 0:
            ...
            for shift in shift_order:
                ...
                if shift not in st.session_state.results_cache:
                    with st.spinner(f"Searching {shift}... Final Master Engine Running!"):
                        ...
                        if base_tier == 'FAIL': base_tier = 'H' # Safety net
                        ...
                        if last_processed_shift and last_shift_tier:
                            linked_tier, linked_prob = calculate_tier_link(shift, last_processed_shift, last_shift_tier, filtered_df)
                            if linked_tier and linked_tier != 'FAIL' and linked_prob > 45.0:
                        ...
                        # SAFE Calculation of Gold & Green lists
                        super_vip_nums = [n for n in nxt if n not in traps and n not in doomed_black_boxes]
                        raw_tier_list = tiers.get(final_tier, tiers['H'])
                        
                        # Combine pure green and vips safely without error
                        safe_green_list = [n for n in raw_tier_list if n not in traps and n not in doomed_black_boxes]
                        pure_green_nums = list(set(safe_green_list) | set(super_vip_nums))
                        ...
                        st.session_state.results_cache[shift] = {
                            ...
                            'traps': list(traps), 'black_boxes': doomed_black_boxes, 'raw_tier_nums': raw_tier_list,
                            'super_vips': super_vip_nums, 'pure_green': pure_green_nums
                        }

                # Retrieve & Display
                res = st.session_state.results_cache[shift]
                
                prev_shift_decisions.append({'shift': shift, 'tf': res['tf'], 'hit_dates': res['hit_dates']})
                ...
                with c2:
                    st.markdown(f"<div style='border:2px solid #00FF7F; padding:10px; border-radius:8px; background:#00FF7F15; font-size:14px;'>"
                                ...
                                f"✅ <b>HARA:</b> {len([n for n in res['pure_green'] if n not in res['super_vips']])} Nums | 🥇 <b>SUNEHRA:</b> {len(res['super_vips'])} Nums | ⬛ <b>KAALA:</b> {len([n for n in res['raw_tier_nums'] if n in res['black_boxes']])} Nums"
                                f"</div>", unsafe_allow_html=True)

                st.markdown(render_ank(res['raw_tier_nums'], res['traps'], res['black_boxes'], res['super_vips']), unsafe_allow_html=True)
